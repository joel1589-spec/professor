...
class SiteSetting(models.Model):
    ...
    hero_text = models.TextField(default='Découvrez les ouvrages, recueils, documents et prestations intellectuelles d’un professeur, chercheur et écrivain engagé pour la connaissance.')
    biography = models.TextField(default="Estino Le Passionné, de son vrai nom AFE Komi, est professeur, écrivain et passionné de transmission du savoir. Son travail valorise la lecture, l’écriture, la pédagogie et les ressources éducatives accessibles.")
...
class Book(models.Model):
    ...
    digital_file = models.FileField(upload_to='documents/', blank=True, null=True)
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            base = slugify(self.title) or str(uuid.uuid4())[:8]
            slug = base
            i = 2
...
class Order(models.Model):
    ...
    PAYMENT = [('MOBILE_MONEY', 'Mobile Money'), ('MANUAL', 'Paiement manuel')]
